...
//
//        mockMvc.perform(get("/api/v1/shop/customers/{customerId}",
// customerId).contentType(MediaType.APPLICATION_JSON))
//                .andExpect(status().isOk())
//                .andExpect(jsonPath("$.id").value(customerId.toString()))
//                .andExpect(jsonPath("$.firstName").value("John"))
//                .andExpect(jsonPath("$.lastName").value("Doe"))
...
//
//        final Product firstProduct = ShopTestFactory.createPremiumProduct("first", Brand.GMX, new
// BigDecimal("100.0"));
//
